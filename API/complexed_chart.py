...
def datetime_date(year: int, month: int, index: int = 0) -> datetime:
    ...
    # month가 1 ~ 12 사이, year가 1900에서 9999 사이 아닐때 None 반환
        return None, None
...
def fetch_recent_100_data() -> list:
    '''DB의 idx필드의 최신 데이터 기준 내림차순 100개를 반환 쿼리'''
    query = '''
...
def fetch_hourly_data(checkdate: datetime) -> list:
    '''DB의 선택한 날짜의 시간 데이터를 반환 쿼리'''
    datequery = """
...
def fetch_weekly_data(checkdate: datetime) -> list:
    '''DB의 선택한 주간 데이터를 반환 쿼리'''
    datequery = '''
...
def fetch_monthly_data(checkdate: datetime) -> list:
    '''DB의 선택한 월간 데이터를 반환 쿼리'''
    datequery = '''
        WHERE date(created_at) BETWEEN date('{}')
        AND date('{}', '+30 days')
        GROUP BY date(created_at)
...
def execute_query(query) -> list:
    '''쿼리 실행 함수'''
    try:
        with sqlite3.connect('JMSPlant.db', check_same_thread=False) as conn:
            cursor = conn.cursor()
...
async def post_hourly_sensor_data(request_data: DataRequest):
    ...
    rows = fetch_hourly_data(checkdate=request_data.date)
    if rows:
...
async def post_data(request_data: WeekDataRequest):
    ...
    try:
        ...
        rows = fetch_weekly_data(checkdate=start_date)
...
async def post_month_data(request_data: MonthDataRequest):
    ...
    try:
        ...
        rows = fetch_monthly_data(checkdate=start_date)
        data = datetime_days(date_list, rows)
        return JSONResponse(content=data)
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))
